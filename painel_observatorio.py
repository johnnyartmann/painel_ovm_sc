...
if not df_geral.empty and not df_feminicidio.empty and geojson_sc is not None:
    ...
    with tab_feminicidio:
        ...
        localidade_crime = df_feminicidio_filtrado['localidade'].value_counts()
        fig_localidade = px.bar(
            localidade_crime, x=localidade_crime.index, y=localidade_crime.values,
            labels={'x': 'Localidade', 'y': 'Quantidade'}, template='plotly_white', text=localidade_crime.values
        )
        fig_localidade.update_traces(marker_color='#8A2BE2', textposition='outside')
        ...
        fig_mes_ano = px.bar(
            feminicidios_por_mes, x='ano_mes', y='quantidade',
            labels={'ano_mes': 'Mês/Ano', 'quantidade': 'Quantidade'},
            template='plotly_white', text='quantidade'
        )
        fig_mes_ano.update_traces(marker_color='#8A2BE2', textposition='outside')
        ...
        feminicidios_por_ano = df_feminicidio_filtrado['ano'].value_counts().sort_index()
        fig_ano_fem = px.bar(
            feminicidios_por_ano, x=feminicidios_por_ano.index, y=feminicidios_por_ano.values,
            labels={'x': 'Ano', 'y': 'Quantidade'}, template='plotly_white', text=feminicidios_por_ano.values
        )
        fig_ano_fem.update_traces(marker_color='#6a1b9a', textposition='outside')
