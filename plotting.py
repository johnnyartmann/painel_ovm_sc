...
def plot_distribuicao_idade(df_idade, chart_type, x_axis_label, color):
    """Gera o gráfico de distribuição de idade."""
    if chart_type == "Histograma":
        fig = px.histogram(df_idade, x=df_idade.columns[0], nbins=20,
                           labels={df_idade.columns[0]: x_axis_label, 'count': 'Quantidade'},
                           template='plotly_white', color_discrete_sequence=[color])
    else:
        fig = px.violin(df_idade, y=df_idade.columns[0],
                        labels={df_idade.columns[0]: x_axis_label},
